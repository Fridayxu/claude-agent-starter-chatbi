...
> ⚠️ **Known Model Compatibility Issue (2026-07)**: The Claude Agent SDK's internal CLI rejects `@makers/` prefixed model names from EdgeOne AI Gateway. For built-in free models (`@makers/deepseek-v4-flash` etc.), use the **Gateway Direct Pattern** instead. See the main SKILL.md "Claude Agent SDK Model Compatibility Issue" section for the working solution.

> When using a bound Anthropic API Key (BYOK with real Claude models like `claude-sonnet-5-20251001`), this route works normally.
