...
## ⚠️ Claude Agent SDK Model Compatibility Issue (2026-07)

**Known issue**: Claude Agent SDK's internal CLI subprocess validates model names against Anthropic's known model list. EdgeOne AI Gateway's built-in free models (`@makers/deepseek-v4-flash`, `@makers/hy3-preview`, `@makers/minimax-m2.7`) use a `@makers/` prefix that the CLI does not recognize, causing `"There's an issue with the selected model"` errors. Removing the prefix causes the gateway to reject with `"Model ID must include provider prefix"`.

**Solution: Gateway Direct Pattern** — bypass the Claude Agent SDK entirely and call the EdgeOne AI Gateway's OpenAI-compatible endpoint via HTTP. This is the **recommended approach** when using EdgeOne's built-in free models.

| Approach | Model Name | Works? |
|----------|-----------|--------|
| Claude Agent SDK | `@makers/deepseek-v4-flash` | ❌ CLI rejects prefix |
| Claude Agent SDK | `deepseek-v4-flash` | ❌ Gateway rejects bare name |
| **HTTP Gateway Direct** | **`@makers/deepseek-v4-flash`** | **✅ Works** |

### Gateway Direct Pattern (Python)

```python
# agents/chat/index.py — tested and working on EdgeOne Makers
import json, httpx
...
### edgeone.json for Gateway Direct

```json
{
  "agents": { "framework": "claude-agent-sdk", "timeout": 600 }
}
```

### requirements.txt for Gateway Direct
...
- [ ] `AI_GATEWAY_BASE_URL` must end with `/v1` (e.g., `https://ai-gateway.edgeone.link/v1`)
- [ ] `AI_GATEWAY_MODEL` uses `@makers/` prefix (e.g., `@makers/deepseek-v4-flash`)
- [ ] `AI_GATEWAY_API_KEY` set in EdgeOne console environment variables
- [ ] NO `os.environ` access — use `ctx.env` (platform rule)
- [ ] Agent handler returns `ctx.utils.stream_sse(gen())` for SSE streaming
- [ ] Preview URL requires `eo_token` and `eo_time` query params — use cookie jar pattern for API calls
