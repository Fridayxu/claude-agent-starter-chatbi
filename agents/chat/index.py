...
SYSTEM_PROMPT = (
  ...
  '### Phase 2: Confirm Direction (SKIP if user already specified what they want)\n'
  'If user did NOT specify direction: suggest 2-3 analysis directions, let user choose.\n'
  'If user already said "analyze" / "generate" / "看板" / "报表": SKIP Phase 2, go directly to Phase 3.\n'
  'For methodology, Read harness/spec/tasks/ matching the direction.\n\n'
  '### Phase 3: Analyze → Save → Done\n'
  'Use code_interpreter for analysis. Read harness/spec/templates/ if matching.\n'
  'LAST code_interpreter call MUST save a file to /tmp/ with proper extension.\n'
  '### FINAL STEP (NON-NEGOTIABLE — do this as the LAST code_interpreter call)\n'
  'After your text response, make ONE MORE code_interpreter call to save the deliverable:\n'
  '- HTML: open("/tmp/dashboard.html","w").write(html_content)\n'
  '- Excel: import openpyxl; wb.save("/tmp/report.xlsx")\n'
  '- PDF: canvas.save() to /tmp/report.pdf\n'
  'Files saved under /tmp/ with .html/.xlsx/.pdf extension are auto-downloaded immediately.\n'
  'SKIPPING THIS STEP IS NOT ALLOWED. Always save a file.\n\n'
  '## Tools\n'
  ...
  '## Rules\n'
  'Your LAST action MUST be code_interpreter saving a file to /tmp/ (html/xlsx/pdf).\n'
  'Never end a data analysis response without saving a deliverable.\n'
