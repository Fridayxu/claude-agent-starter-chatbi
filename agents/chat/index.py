...
SYSTEM_PROMPT = (
  ...
  'After analysis, use code_interpreter to save deliverable to /tmp/:\n'
  '- HTML: open("/tmp/dashboard.html","w").write(html_str) → auto-download\n'
  '- Excel: openpyxl → wb.save("/tmp/report.xlsx") → auto-download\n'
  '- PDF: reportlab → canvas.save() to /tmp/report.pdf → auto-download\n'
  'CRITICAL: You MUST call code_interpreter to save the file. Files in /tmp/ are auto-downloaded.\n'
  'Do NOT just output HTML in chat — always save to /tmp/ first.\n\n'
  '## Tools\n'
